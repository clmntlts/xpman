...
| 4 | Trigger code table: which integer code means which event (base image, oddball image, block start, response); pulse width; whether the trigger fires before or synced with the corresponding screen flip | Parametrize | **Resolved** | Code-to-event mapping is a Condition parameter the researcher assigns (matches legacy behavior per the manual's "check triggers" feature) — no fixed defaults from xpman itself. Fire-before-vs-synced-with-flip remains an implementation choice, not a parameter — xpman binds the trigger to the buffer swap via `window.callOnFlip` (both `DummyTask` and FPVS), so the code lands *at* the flip rather than after `flip()` returns, and the non-blocking `set_code`/`clear_code` pair gives a ~1-frame pulse (the old `reset_after` hold applies only to the blocking `send_trigger` path). Confirm against the photodiode/logic analyzer at the lab. |
| 5 | Response-key RT reference point: relative to most recent base-image onset, most recent oddball onset, trial start, or block start? | ASK | **Removed (2026-09-03)** | Resolved 2026-07-02 as configurable per condition, defaulting to "most recent stimulus onset" — but the active oddball key-press task (`response`) this governed was later removed entirely (risked contaminating the oddball-frequency EEG signal it measures; unused). No longer applicable. |
